            ...
            <div class="container-fluid">
              <div class="row row-cols-4">
                ...
                <div class="col-sm-2 col-lg-2">
                  ...
                  <select id="status" name="status" class="form-control selectpicker" data-style="ripple-none select-with-transition" data-width="100%" title="-- กรุณาเลือก --">
                  </select>
                </div>
                <div class="col-4">
...
<script type="text/javascript">
  ...
  var fn = {
    getData: function() {
      ...
      $.ajax({
        type: 'GET',
        // url: base_url('/hr_recruit/hr/hrList/getData'),//page,
        url: base_url('/hr/hrList/getData'), // use test
        dataType: 'json',
    ...
    setTable: function(dataset = [], tableId = "") {
      ...
      $("#" + tableId + " tbody").on('dblclick', 'tr', function() {
        ...
        // window.open(base_url('/hr_recruit/hr/detail_hr?id=' + data.id), "_blank");
        window.open(base_url('/hr/detail_hr?id=' + data.id), "_blank");
      });
    },
    ...
    find: function() {
      ...
      $.ajax({
        type: 'GET',
        // url: base_url('/hr_recruit/hr/hrList/getData'),//base_url("<?php echo "/" . $page; ?>/getData"),
        url: base_url('/hr/hrList/getData'), // use test
        data: {
    ...
    getMaster: function(params) {
      ...
      $.ajax({
        type: 'GET',
        // url: base_url('/hr_recruit/hr/hrList/getMaster'),//base_url("<?php echo "/" . $page; ?>/getMaster"),
        url: base_url('/hr/hrList/getMaster'), // use test
        data: {
        ...
        success: function(res, textStatus, jqXHR) {
          ...
          if (params != "status") {
            select_status = '<option value="">ไม่เลือก</option>';
            $.each(res.success.data.status, function(index, value) {
              select_status += '<option value="' + value.status + '">' + value.status + '</option>';
            });
            $("#status").html(select_status).selectpicker('refresh');
